...
public class GreedyPartialOrderLifter implements PartialOrderLifter {
	...
	private GreedyPartialOrderLifter() {
		throw new IllegalStateException("Utility class");
	}

	// This could probably make use of the data structures in the relaxed plan graph
	public static PartialOrderPlan lift(TotalOrderPlan p, GroundProblem problem) {
		/*
		...
		pop.addActions(p.getActions());

		Set goals = new HashSet(problem.goal.getConditionalPropositions());

		ListIterator toit = p.listIteratorEnd();
		while (toit.hasPrevious()) {
			Action a = (Action) toit.previous();

			getAOrderings(a, p, pop);
			getBOrderings(a, p, pop);
			getCOrderings(a, p, pop, goals);
	...
	public static void getAOrderings(Action a, TotalOrderPlan p, PartialOrderPlan pop) {
		Iterator cit = a.getConditionalPropositions().iterator();
		while (cit.hasNext()) {
			Proposition c = (Proposition) cit.next();
			ListIterator bit = p.listIterator(a);
	...
	public static void getBOrderings(Action a, TotalOrderPlan p, PartialOrderPlan pop) {
	...
	public static void getCOrderings(Action a, TotalOrderPlan p, PartialOrderPlan pop, Set goals) {
